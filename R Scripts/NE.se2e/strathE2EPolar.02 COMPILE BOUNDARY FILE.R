...
Boundary_new <- Boundary_template %>% 
  mutate(SO_nitrate = My_boundary_data %>% filter(Compartment == "Offshore S") %>% .$NO3,
         ...
         # RIV_ammonia = My_river_N$NH4, # don't have this data for EG so just use NM from Jack's paper
         RIV_detritus = 0,
...
write.csv(Boundary_new, file = paste0("C:/Users/psb22188/AppData/Local/R/win-library/4.5/StrathE2EPolar/extdata/Models/East_Greenland/",start_year,"-",end_year,"-",forcing,"-SSP",ssp,"/Driving/chemistry_EG_",start_year,"-",end_year,"-",forcing,"-SSP",ssp,".csv"),
          row.names = F)
