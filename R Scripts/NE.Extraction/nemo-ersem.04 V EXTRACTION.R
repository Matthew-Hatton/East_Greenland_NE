...
domain <- readRDS("./Objects/domain/Domains.rds") %>%                              # Get the horizontal area to extract over 
  select(Shore) %>% 
...
Bathymetry <- readRDS("./Objects/domain/NE_grid.rds") %>%                          # Import NEMO-MEDUSA bathymetry
...
scheme <- scheme_interp_slice(get_spatial(example, depthdim = "depth"), s_depth, domain) # Get a scheme for linear interpolation between 2 depth layers
...
scheme <- scheme_reframe(scheme) %>%                                        # Adjust scheme indices so they match the array subset
  ...
  filter(depth < Bathymetry & s_depth < Bathymetry) %>%                      # Drop points where the target depth or next deeper layer are below the sea floor
...
summary <- filter(scheme, layer == 1) %>%                                   # Create the metadata to attach to summaries
  ...
  mutate(depth = s_depth) %>%                                                # Lets return the depth we interpolated to
...
#### check ####
# 
# NE <- readRDS("./Objects/vertical boundary/NE.CNRM.hist.01.1991.rds") %>%
