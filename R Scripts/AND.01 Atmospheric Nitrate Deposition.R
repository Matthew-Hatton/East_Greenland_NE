...
domains <- readRDS("./Objects/domain/Domains.rds") %>%                             # Import inshore polygon
  st_transform(crs = 4326)

## raw files stored in another directory
...
ggsave("./Figures/Atmospheric N Deposition.png", last_plot(), dpi = 500, width = 18, height = 10 , units = "cm", bg = "white")

#### Save ####
Deposition_ssp %>% 
  dplyr::select(Month, Oxidation_state, Shore,  Year, SSP, Measured) %>%  
  saveRDS("./Objects/chemistry/NE.Atmospheric N deposition.rds")
